...
class spiritsupc
{
	...
	function getupc1()
	{
		$query = "SELECT * FROM upc WHERE sku = :sku AND upc = :upc";
		$stmt = $this->conn->prepare($query);
	...
	function updatesdateUpc1()
	{
		...
		$query = "update upc set `last` = :last,`level` = :level,tstamp = now(),lstore =:lstore,lreg= :lreg,who = :who, isdel = :isdel where sku = :sku and upc = :upc";
		$stmt = $this->conn->prepare($query);
		$this->upc = strip_tags($this->upc);
		$this->last = strip_tags($this->last);
