...
class spiritsstk{
	...
	function updateJnlStk(){
		...
		$query = "INSERT INTO stk (sku,store,floor,shipped,kits,sdate,who,tstamp,isdel,lreg,lstore)
			VALUES 
		(:sku,:store,floor + :floor,shipped + :shipped,kits + :kits,:sdate,:who,now(),:isdel,:lreg,:lstore)
		ON DUPLICATE KEY UPDATE
		floor = floor + :floor,
		shipped = shipped + :shipped,
		kits = kits + :kits,
