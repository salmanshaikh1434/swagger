<?php
	header("Access-Control-Allow-Origin: *");
	header("Content-Type: application/json; charset=UTF-8");
	header("Access-Control-Allow-Methods: POST");
	header("Access-Control-Max-Age: 3600");
	header("Access-Control-Allow-Headers: Content-Type, Access-Control-Allow-Headers, Authorization, X-Requested-With");

	include_once '../config/database.php';
	include_once '../objects/spiritscat.php';

	$database = new Database();
	$db = $database->getConnection();

	$spiritsUPDB = new spiritscat($db);

	$data = json_decode(file_get_contents("php://input"));

		$spiritsUPDB->cat = $data->cat;
		$spiritsUPDB->name = $data->name;
		$spiritsUPDB->seq = $data->seq;
		$spiritsUPDB->lspace = $data->lspace;
		$spiritsUPDB->code = $data->code;
		$spiritsUPDB->cost_plus = $data->cost_plus;
		$spiritsUPDB->cost = $data->cost;
		$spiritsUPDB->taxlevel = $data->taxlevel;
		$spiritsUPDB->sur = $data->sur;
		$spiritsUPDB->disc = $data->disc;
		$spiritsUPDB->dbcr = $data->dbcr;
		$spiritsUPDB->cflag = $data->cflag;
		$spiritsUPDB->income = $data->income;
		$spiritsUPDB->cog = $data->cog;
		$spiritsUPDB->inventory = $data->inventory;
		$spiritsUPDB->discount = $data->discount;
		$spiritsUPDB->who = $data->who;
		$spiritsUPDB->wsgroup = $data->wsgroup;
		$spiritsUPDB->wscod = $data->wscod;
		$spiritsUPDB->wsgallons = $data->wsgallons;
		$spiritsUPDB->catnum = $data->catnum;
		$spiritsUPDB->fson = $data->fson;
		$spiritsUPDB->fsfactor = $data->fsfactor;
		$spiritsUPDB->datacap = $data->datacap;
		$spiritsUPDB->sent = $data->sent;
		$spiritsUPDB->belowcost = $data->belowcost;
		$spiritsUPDB->gosent = $data->gosent;
		$spiritsUPDB->lreg= $data->lreg;
		$spiritsUPDB->lstore= $data->lstore;
		$spiritsUPDB->isdel = $data->isdel;

	$spiritsRTN = $spiritsUPDB->updateCat();
	if ($spiritsRTN == false) {
		echo json_encode(
			array("message" => "Unauthorised request.")
		);
		exit;
	}
	if($spiritsRTN=='OK'){
	    http_response_code(200);
	    echo json_encode(array("message" => "UPDATED.."));
	}
	else{
   		http_response_code(503);
		echo json_encode(array("message" => "ERROR UPDATING!!"));
    	echo json_encode(array("error" =>$spiritsRTN));
	}
?>
