...
// rename Pod to data and removed foreach by salman
$data = json_decode(file_get_contents("php://input"));

function is_blank_object($obj)
{
	if (!is_object($obj)) {
		return false;
	}
	foreach ($obj as $property) {
		if (is_object($property) && !is_blank_object($property)) {
			return false;
		}
...
if (!isset($data) || is_blank_object($data) || !property_exists($data, 'order') || !property_exists($data, 'store') || !property_exists($data, 'line')) {
